...
_NAME_RE = re.compile(r"(.*)(speech|music)\_?.*\/[\w-]*\.(wav|au)")


class GTZANMusicSpeechConfig(tfds.core.BuilderConfig):
  """Configuration Class for PAWS - X Dataset."""

  def __init__(self, *, file_format, **kwargs):
    super(GTZANMusicSpeechConfig, self).__init__(**kwargs)
  ...
  def _info(self):
    return tfds.core.DatasetInfo(
        ...
        features=tfds.features.FeaturesDict({
            "audio":
                tfds.features.Audio(
                    file_format=self.builder_config.file_format,
                    sample_rate=22050),
  ...
  def _generate_examples(self, path):
    ...
    for root, _, file_name in tf.io.gfile.walk(path):
      for fname in file_name:
        if fname.endswith(self.builder_config.file_format):
          full_file_name = os.path.join(root, fname)
          res = _NAME_RE.match(full_file_name)
          label = res.group(2)
          key = fname
          example = {
